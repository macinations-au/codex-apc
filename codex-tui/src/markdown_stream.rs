...
fn should_collapse_think(config: &Config) -> bool {
...
fn collapse_think_blocks(s: &str) -> String {
    let mut out = String::with_capacity(s.len());
    let mut i = 0;
    while let Some(start_rel) = s[i..].find("<think>") {
        let start = i + start_rel;
        out.push_str(&s[i..start]);
        if let Some(end_rel) = s[start + 7..].find("</think>") {
            i = start + 7 + end_rel + 8; // skip over </think>
        } else {
            // No closing tag; break to avoid dropping tail
            i = start;
            break;
        }
    }
