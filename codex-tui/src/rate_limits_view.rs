...
impl RateLimitMetrics {
    fn from_snapshot(snapshot: &RateLimitSnapshotEvent) -> Self {
        ...
        Self {
            hourly_used,
            weekly_used,
            hourly_remaining: (100.0_f64 - hourly_used).max(0.0),
            weekly_remaining: (100.0_f64 - weekly_used).max(0.0),
            hourly_window_label: format_window_label(Some(snapshot.primary_window_minutes)),
            weekly_window_label: format_window_label(Some(snapshot.secondary_window_minutes)),
            hourly_reset_hint: format_reset_hint(Some(snapshot.primary_window_minutes)),
            weekly_reset_hint: format_reset_hint(Some(snapshot.secondary_window_minutes)),
        }
    }
