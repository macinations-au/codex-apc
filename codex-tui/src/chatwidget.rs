...
impl ChatWidget {
    ...
    pub(crate) fn open_model_popup(&mut self) {
        ...
        // Downstream enhancement: when using the built-in OSS provider, append
        // locally available Ollama models to the picker so `/model` can choose them.
        if self.config.model_provider_id == codex_core::BUILT_IN_OSS_MODEL_PROVIDER_ID {
            // Query Ollama synchronously using the current runtime handle to avoid
            // creating a nested runtime. Failures are non-fatal.
    ...
    pub(crate) fn open_reasoning_popup(&mut self) {
        let mut items: Vec<SelectionItem> = Vec::new();
        ...
        self.bottom_pane.show_selection_view(
            ...
            Some("Choose how to display the model’s thinking".to_string()),
            Some("Press Enter to confirm or Esc to go back".to_string()),
            items,
        );
    }
