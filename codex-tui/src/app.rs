...
fn built_in_provider_by_id(id: &str) -> Option<codex_core::ModelProviderInfo> {
    ...
    let map = codex_core::built_in_model_providers();
    map.get(id).cloned()
}
    ...
    async fn handle_event(&mut self, tui: &mut tui::Tui, event: AppEvent) -> Result<bool> {
        match event {
            ...
            AppEvent::FooterNotice(text) => {
                self.chat_widget
                    .set_footer_notice(text, std::time::Duration::from_secs(60));
            }
            ...
            AppEvent::UpdateReasoningView(view) => {
                match view {
            ...
            AppEvent::UpdateModelProvider(provider_id) => {
                if let Some(provider) = built_in_provider_by_id(&provider_id) {
...
// Removed: persist_provider — provider is session-only.

async fn persist_reasoning(
    codex_home: &std::path::Path,
    active_profile: Option<&str>,
    view: crate::cli::ReasoningViewArg,
) -> anyhow::Result<()> {
    ...
    tbl.insert("hide_agent_reasoning", value(hide));
    tbl.insert("show_raw_agent_reasoning", value(show_raw));
